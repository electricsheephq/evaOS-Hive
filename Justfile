...
#
# The full desktop Tauri suite runs under the managed feature.
desktop-tauri-managed-check: _ensure-sidecar-stubs
    ...
    cd desktop/src-tauri && cargo test --features evaos-teams-managed
...
desktop-release-build target="aarch64-apple-darwin":
    ...
    pnpm install
    cd {{desktop_dir}} && pnpm tauri build --features mesh-llm,evaos-teams-managed --target {{target}}
