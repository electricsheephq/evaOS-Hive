...
export function installDesktopProductPolicy(
  ...
  if (
    policy.managed &&
    (policy.updaterEnabled || policy.upstreamHostedServicesEnabled)
  ) {
    throw new Error("Managed desktop product policy is not fail-closed");
  }
  activeProductPolicy = Object.freeze({ ...policy });
}

export async function loadDesktopProductPolicy(): Promise<void> {
  const policy = await invoke<DesktopProductPolicy>(
    "get_desktop_product_policy",
  );
  installDesktopProductPolicy(policy);
}
